...
class SystemMonitor:
    ...
    def get_stats(self) -> dict:
        ...
        # Active window (Linux — X11 via xdotool, Wayland via hyprctl/swaymsg)
        try:
            ...
            if session_type == "wayland":
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        stats["active_window"] = win_data.get("title", "")
                except (FileNotFoundError, Exception):
                    pass
                # Try swaymsg (Sway)
                if not stats["active_window"]:
                    try:
                        result = _sp.run(
                            ["swaymsg", "-t", "get_tree"],
                            capture_output=True, text=True, timeout=3,
                        ...
                        if result.returncode == 0:
                            ...
                            if focused:
                                stats["active_window"] = focused.get("name", "")
                    except (FileNotFoundError, Exception):
                        pass
            else:
                # X11 — xdotool
                ...
                if result.returncode == 0:
                    stats["active_window"] = result.stdout.strip()
        except Exception:
            pass
...
class ScreenCapture:
    ...
    def _capture_and_describe(self) -> str | None:
        """Take screenshot using Linux tools, send to vision LLM."""
        try:
            import subprocess
            import tempfile
            import base64
            ...
            if session_type == "wayland":
                ...
                try:
                    ...
                    print("[Screen] grim not found. Install: sudo pacman -S grim")
            else:
                # X11 — try scrot, then maim, then import (ImageMagick)
                for cmd in [
                    ["scrot", tmp_path],
                    ["maim", tmp_path],
            ...
            if not captured:
                try:
                    ...
                    pass
                print("[Screen] No screenshot tool available")
                self._enabled = False
                return None
            ...
            print("[Screen] Also need: grim (Wayland) or scrot/maim (X11)")
            self._enabled = False
        except Exception as e:
...
class Soul:
    def __init__(self, config):
        ...
        self._last_health_alert = 0.0  # GPU/CPU alert cooldown
    ...
    def save(self):
        ...
        try:
            # Atomic write via temp file (matches memory.py)
            tmp = SAVE_PATH + ".tmp"
    ...
    def adopt_history(self, total_messages: int, first_interaction: str):
        """Seed relationship stats from PersistentMemory on the soul's first
        run, so a long-standing friendship isn't greeted with milestone
        fanfare ("our first conversation!") it already lived through."""
        with self._lock:
            s = self.state
            if s.total_interactions > 0 or total_messages <= 0:
                return
            s.total_interactions = total_messages
            # _load() stamps first_met = today on a fresh soul, so take the
            # EARLIER of the two dates — memory's record predates the soul.
            if first_interaction:
                try:
                    mem_first = datetime.fromisoformat(first_interaction).date()
                    current = None
                    if s.first_met:
                        try:
                            current = datetime.fromisoformat(s.first_met).date()
                        except ValueError:
                            pass
                    if current is None or mem_first < current:
                        s.first_met = mem_first.isoformat()
                    s.total_days_known = (
                        datetime.now().date()
                        - datetime.fromisoformat(s.first_met).date()).days
                except ValueError:
                    pass
            # Mark already-passed milestones silently (no retroactive fanfare)
            for key, cond in (
                ...
                if cond and key not in s.milestones:
                    s.milestones.append(key)
            print(f"[Soul] Adopted history: {s.total_interactions} messages "
                  f"since {s.first_met or '?'}")
        self.save()
    ...
    def _tick(self):
        ...
        with self._lock:
            ...
            # Active-app tracking (works without screenshots too)
            active = sys_stats.get("active_window", "")
            if active:
                if active != s.last_active_app:
                    s.last_active_app = active
                    s.active_app_start = now
                elif s.active_app_start and (now - s.active_app_start) > 7200:
            ...
            # System health alerts (cooldown so a hot GPU doesn't nag
            # every tick for the whole session)
            if now - self._last_health_alert > 1800:
